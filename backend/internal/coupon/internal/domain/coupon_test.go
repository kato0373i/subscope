...
func TestRedeem_EnforcesLimit(t *testing.T) {
	c, _ := New("CPN-1", "ORG-1", "LTD", DiscountAmount, 500, 2)
	if err := c.Redeem("BA-1"); err != nil {
...
func TestApply_PercentAndAmountAndFloor(t *testing.T) {
	pct, _ := New("CPN-1", "ORG-1", "P10", DiscountPercent, 10, 0)
	if got, _ := pct.Apply(shared.JPY(3000)); got.Amount != 2700 {
		t.Errorf("10%%引き = %d, want 2700", got.Amount)
	}

	amt, _ := New("CPN-2", "ORG-1", "A500", DiscountAmount, 500, 0)
	if got, _ := amt.Apply(shared.JPY(3000)); got.Amount != 2500 {
		t.Errorf("500 円引き = %d, want 2500", got.Amount)
	}

	// 割引が額面を超えても 0 円止まり。
	big, _ := New("CPN-3", "ORG-1", "A9999", DiscountAmount, 9999, 0)
